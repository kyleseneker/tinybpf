...
func injectBTF(ctx context.Context, cfg Config, tools llvm.Tools) error {
	if tools.Pahole == "" {
		return diag.New(diag.StageBTF,
			fmt.Errorf("pahole not found"), "pahole", "",
			"install pahole or pass --pahole when using --btf")
	}
	...
	if err != nil {
		return diag.New(diag.StageBTF, err, res.Command, res.Stderr,

...
func TestRewriteCoreAccessMalformedGEP(t *testing.T) {
	lines := []string{
		"%main.bpfCoreTaskStruct = type { i32, i32 }",
		"define void @handler(ptr %ctx) {",
		"  %p = getelementptr %main.bpfCoreTaskStruct, ptr %base",
		"}",
	}
	_, err := rewriteCoreAccess(lines)
	if err == nil {
		t.Fatal("expected error for malformed GEP")
	}
	if !strings.Contains(err.Error(), "does not match expected GEP pattern") {

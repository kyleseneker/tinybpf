...
```go
//go:extern bpf_kfunc_bpf_iter_task_next
func bpfKfuncBpfIterTaskNext(it unsafe.Pointer) unsafe.Pointer
```
...
| Convention | Details |
|------------|---------|
| Prefix | `bpfKfunc` -- e.g. `bpfKfuncBpfIterTaskNext` |
...
- **Iterator programs require kfuncs.** BPF iterators rely on `bpf_iter_*` kfuncs. Basic kfunc support is available (see above), but iterator-specific context struct handling may need additional work.
...
Most types the kernel supports: tracepoints, kprobes, fentry/fexit, XDP, cgroup, socket filters, TC classifiers, LSM hooks, and more. The type is determined by the ELF section name set via `--section` or `build.programs` in config. **struct_ops** is not supported (requires function pointers, which Go cannot express).
...
It parses the IR into an AST and runs an 8-pass transformation: retarget and strip attributes, extract programs, replace allocations, rewrite helpers, CO-RE transforms, assign sections, map BTF encoding, and finalize. See [Architecture](architecture.md) for the full breakdown.

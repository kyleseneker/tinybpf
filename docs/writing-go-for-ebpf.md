...
Any type the kernel supports: tracepoints, kprobes, XDP, cgroup, socket filters, TC classifiers, etc. The type is determined by the ELF section name set via `--section` or `build.programs` in config.

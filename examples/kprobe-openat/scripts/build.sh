...
if [[ "${TINYGO_OUTPUT_FORMAT}" == "ll" ]]; then
  IR_FILE="${BUILD_DIR}/openat.ll"
else
  IR_FILE="${BUILD_DIR}/openat.bc"
fi
...
echo "[3/3] tinybpf -> ${BUILD_DIR}/openat.bpf.o"
"${TINYBPF_BIN}" \
  --input "${IR_FILE}" \
  --output "${BUILD_DIR}/openat.bpf.o" \
  --section kprobe_openat=kprobe/do_sys_openat2 \
  --cpu "${CPU}" \
  --verbose

echo "built object: ${BUILD_DIR}/openat.bpf.o"
